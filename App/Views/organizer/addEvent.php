...
<main class="overflow-x-scroll scrollbar-hide flex flex-col justify-between pt-[42px] px-[23px] pb-[28px]">
    <form action="/router/web.php" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="url" value="addEvent">
        <input type="hidden" name="Event" value="<?=isset($EventEdit)&&!empty($EventEdit)?$EventEdit['id']:'' ?>">

        <div>
          <h2 class="capitalize text-gray-1100 font-bold text-[28px] leading-[35px] dark:text-gray-dark-1100 mb-[13px]"><?=isset($EventEdit)&&!empty($EventEdit)?'Edit Event':'Add new Event' ?></h2>
            ...
            <div class="flex justify-between gap-x-5 flex-col xl:flex-row">
              <div class="xl:w-[70%]">
                <div class="mb-12">
                  ...
                  <p class="text-gray-1100 text-base leading-4 font-medium capitalize mb-[10px] dark:text-gray-dark-1100">Event title</p>
                  <div class="input-group border rounded-lg border-[#E8EDF2] dark:border-[#313442] sm:min-w-[252px]">
                    <input class="input bg-transparent text-sm leading-4 text-gray-400 h-fit min-h-fit py-4 focus:outline-none pl-[13px] dark:text-gray-dark-400 placeholder:text-inherit" name="title" type="text" value="<?=isset($EventEdit)&&!empty($EventEdit)?$EventEdit['title']:'' ?>" placeholder="Add title">
                  </div>
                </div>
                <p class="text-gray-1100 text-base leading-4 font-medium capitalize mb-[10px] dark:text-gray-dark-1100">Event description</p>
                <div class="rounded-lg border border-neutral dark:border-dark-neutral-border p-[13px]">
                    <textarea class="textarea w-full p-0 text-gray-400 resize-none rounded-none bg-transparent focus:outline-none" name="description" placeholder="Description here"><?=isset($EventEdit)&&!empty($EventEdit)?$EventEdit['description']:'' ?></textarea>
                  </div>
                <div class="mb-12">
                  <p class="text-gray-1100 text-base leading-4 font-medium capitalize mb-[10px] dark:text-gray-dark-1100">Event content</p>
                  <textarea id="summernote" name="content" class="bg-black "><?=isset($EventEdit)&&!empty($EventEdit)?$EventEdit['content']:'' ?></textarea> 
                  </div>
                <div class="mb-12"> 
    ...
    <div class="border-dashed border-2 text-center border-neutral py-[26px] dark:border-dark-neutral-border">
        <img class="mx-auto inline-block mb-[15px]" src="/public/assets/images/icons/icon-image.svg" alt="image icon">
        ...
        <input type="file" id="drop-zone" name="image" value="<?=isset($EventEdit)&&!empty($EventEdit)?$EventEdit['Event_img']:'' ?>" class="hidden" accept="image/jpeg, image/png, image/gif, image/webp" />
    </div>
</label>
                </div>
              ...
              <div class="border border-neutral rounded-lg dark:border-dark-neutral-border pb-[31px]">
                  ...
                  <div class="mt-3 md:flex md:items-center md:-mx-2 flex justify-center">
                            <label class="flex items-center">
                                <input type="radio" name="coursType" value="doc" class="hidden peer" id="student" <?=isset($EventEdit)&& !empty($EventEdit) && $EventEdit['type'] == 'document' ? 'checked' : ''?>/>
                            ...
                            <label class="flex items-center mt-4 md:mt-0">
                                <input type="radio" name="coursType" value="video" class="hidden peer" id="organizer" <?=isset($EventEdit)&& !empty($EventEdit) && $EventEdit['type'] == 'video' ? 'checked' : ''?> />
                                <span class="flex items-center justify-center w-full px-6 py-3 text-[#494282] border border-[#494282] rounded-lg cursor-pointer md:w-auto md:mx-2 dark:border-red-400 dark:text-red-400 hover:bg-[#494282] hover:text-white transition duration-200 peer-checked:bg-[#494282] peer-checked:text-white">
                ...
                <div class="border border-neutral rounded-lg bg-neutral-bg dark:border-dark-neutral-border pb-[31px] dark:bg-dark-neutral-bg">
                  ...
                  <div class="flex flex-col pl-[18px] gap-y-[15px] mb-[25px]">
                    <div class="flex items-center gap-x-[6px]"><img src="/public/assets/images/icons/icon-tree.svg" alt="tree icon"><span class="text-gray-500 text-xs dark:text-gray-dark-500">Status:</span><span class="text-gray-1100 text-xs dark:text-gray-dark-1100">Draft</span></div>
                    <div class="flex items-center gap-x-[6px]"><img src="/public/assets/images/icons/icon-eye.svg" alt="eye icon"><span class="text-gray-500 text-xs dark:text-gray-dark-500">Visibility:</span><span class="text-gray-1100 text-xs dark:text-gray-dark-1100"><?=isset($EventEdit)&&!empty($EventEdit)?$EventEdit['STATUS']:'pending' ?></span></div>
                    <div class="flex items-center gap-x-[6px]"><img src="/public/assets/images/icons/icon-calendar-1.svg" alt="calendar icon"><span class="text-gray-500 text-xs dark:text-gray-dark-500">Schedule:</span><span class="text-gray-1100 text-xs dark:text-gray-dark-1100">Immediately</span></div>
                  </div>
